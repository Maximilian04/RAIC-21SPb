	...
	if (!prodCycle.isPlanned) {
		...
		if(buildingOrder.size() == 0) //if buildingOrder is not initialized
		{
			for(int i = 0; i < prodCycle.buildingPlanet.size(); i++)
			{
				for(int j = 0; j < prodCycle.buildingPlanet[i].size(); j++)
				{
					buildingOrder.push_back(pair<int,int>(prodCycle.buildingPlanet[i][j], i)); //planet id and building type
				}
			}
		...
		{
			static vector<bool> inProcess(buildingOrder.size(), false); //true if there is number of workers already in a flight to build the ith building
			int freestone = min(game.planets[homePlanet].resources.count(t2r(STONE)) ?
								game.planets[homePlanet].resources.at(t2r(STONE)) : 0,
								!game.planets[homePlanet].workerGroups.empty() ?
								game.planets[homePlanet].workerGroups[0].number : 0);
			...
			for(int i = 0; i < buildingOrder.size(); i++)
			{
				if(isBuilt[i])
				{
					...
					{
						fc.send(buildingOrder[i].first, homePlanet, game.planets[buildingOrder[i].first].workerGroups[0].number, {}, true); //if there are workers on a planet with a building then order them back
					}
				}
				else if(!inProcess[i]){
					int price = game.buildingProperties.at(t2b(buildingOrder[i].second)).buildResources.at(model::Resource::STONE);
