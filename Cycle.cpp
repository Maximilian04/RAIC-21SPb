					  ...
					  const vector<vector<int>>& planetDists)
{
	...
	buildGraph.resize(game.planets.size()); //graph where objects are planets connected with their 'neighbour' planets with the same type
...
double Cycle::logistsRequired(const vector<vector<int>>& planetDists, const vector<vector<int>>& candidates, const vector<double>& cons)
{
	...
	for(int i = 0; i < 9; i++) 
	{
		for(auto p: trafficCoeff[i])
		{
			...
			double realcost = (cost/candidates[i].size())/candidates[j].size(); //if 2 MINES send 4 res to 2 FOUNDRIES, each MINE sends 1 res to each FOUNDRY
...
vector<double> Cycle::getMaxWorkPower(const model::Game& game)
{
	...
	for(int i = 0; i < 3; i++)
	{
		double curcoeff = (planetTypes[i].size()*game.buildingProperties.at(t2b(i)).maxWorkers)/maxwp[i]; //coeff = theoretically possible 'amount' of work on planets (if they're all 100% used) devided by calculated max
		//cout << curcoeff << '\n';
...
vector<vector<int>> Cycle::positionBuilding(const model::Game& game, const vector<vector<int>>& planetDists, vector<double> buildPower, int attempts, int opts)
{
	...
	for (int att = 0; att < attempts; att++) //making attempts trying to find the best solution
	{
		...
		for (int i = 0; i < current.size(); i++) {
			...
			for(int j = 0; j < current[i].size(); j++)
			{
				...
				else
				{
					...
					{
						//cout << "fail :(\n";
						return vector<vector<int>>(1,vector<int>(1,-1)); //fail (not enough planets to build the base lmao)
					}
				}
			}
		}
		...
		for (int o = 0; o < opts; o++) {
			...
			for (int i = 0; i < current.size(); i++) 
			{
				...
				{
					for(int k = 0; k < buildGraph[current[i][j]].size(); k++) //running through all 'neighbour' planets
					{
						...
						{
							if (indicies[pl].first >= 3 ||
								planetType(game, current[i][j]) == indicies[pl].first) //check if we can swap
							{
								newcand[i][j] = pl;
						 ...
						 const vector<vector<int>>& planetDists)
{
	...
	cout << logistsRequired(planetDists, buildingPlan, bestwp) << " " << bestpower << " " << bestpower/Cycle::baseConsumption/5 << " per tick\n";
